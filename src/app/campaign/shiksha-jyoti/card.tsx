...
const DonationDashboard = () => {
  ...
  // Shiksha Jyoti - SHIKSHA Campaign Data
  const raised = 41437;
  const goal = 1000000;
  const donors = 0;
  const benefited = 0;
  ...
  return (
    <div className="h-[600px] bg-purple-50 flex flex-col items-center justify-center p-6 font-sans">
      ...
      >
        SHIKSHA<span className="text-purple-600"> JYOTI</span>
      </motion.h1>
      ...
      <div className="w-full max-w-5xl flex flex-col lg:flex-row items-stretch shadow-[0_40px_80px_-20px_rgba(124,58,237,0.15)] rounded-[2.5rem] overflow-hidden bg-white border border-purple-100">
        ...
        <div className="w-full lg:w-5/12 bg-purple-600 p-8 md:p-10 flex flex-col justify-between relative overflow-hidden text-white">
          ...
          >
            <span className="text-purple-200 font-bold uppercase tracking-widest text-[10px]">By Swachetna Foundation</span>
            <h2 className="text-3xl font-black mt-2 leading-tight">Every Child Deserves a Brighter Future</h2>
            <p className="mt-3 text-purple-100 leading-relaxed text-sm opacity-90">
              Providing learning centers, school supplies, and mentorship to children from underserved communities in Pune.
            </p>
          </motion.div>
        ...
        <div className="w-full lg:w-7/12 p-8 md:p-12 bg-white flex flex-col justify-center">
          ...
          >
            <div>
              <h2 className="text-3xl font-black text-zinc-900 tracking-tight">Support Education</h2>
              <p className="text-zinc-400 text-sm mt-1 font-medium">Your contribution funds scholarships and learning supplies.</p>
            </div>

            <div className="space-y-4">
              <label className="text-[10px] font-black text-purple-600 uppercase tracking-widest">Select Donation Amount</label>
