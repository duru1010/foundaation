...
const StatCircle = ({ value, label, percent, description, index }: any) => {
  const radius = 70; // Slightly smaller radius for better fit on all devices
  const circumference = 2 * Math.PI * radius;
  const strokeDashoffset = circumference - (percent / 100) * circumference;
  ...
  return (
    <motion.div 
      ...
      className="flex flex-col items-center text-center group"
    >
      {/* Container: w-40 for mobile, w-44 for desktop */}
      <div className="relative w-40 h-40 md:w-44 md:h-44 mb-4 flex items-center justify-center">
        ...
        <div className="z-10 flex flex-col items-center">
          <div className="flex flex-col items-center leading-none">
            <span className="text-[8px] md:text-[9px] uppercase font-bold text-gray-400 mb-1">Total</span>
            <span className="text-xl md:text-2xl font-black text-gray-800 tracking-tighter">{value}</span>
          </div>
          <div className="mt-2 flex flex-col items-center leading-none border-t border-gray-100 pt-2 px-4">
            <span className="text-[8px] md:text-[9px] uppercase font-bold text-purple-600 tracking-widest">{label}</span>
          </div>
        </div>
      </div>

      <h3 className="text-[10px] md:text-[11px] font-black uppercase tracking-[0.2em] text-gray-800 mb-1">
        {label} <span className="text-purple-600">Metric</span>
      </h3>
      <p className="text-[10px] text-gray-400 leading-tight max-w-[150px] md:max-w-[170px]">
        {description}
      </p>
    </motion.div>
  );
...
const BusinessCircleAnalysis = () => {
  ...
  const stats = [
    { label: "Volunteers", value: "500,000", percent: 85, desc: "Community members engaged worldwide." },
    { label: "Donation", value: "5M+", percent: 65, desc: "Total funds raised for global initiatives." },
    { label: "Projects", value: "1,240", percent: 45, desc: "Completed infrastructures and programs." },
    { label: "Countries", value: "Global", percent: 95, desc: "Active presence across all continents." },
  ];
      ...
      <AnimatePresence mode="wait">
        {isLoading ? (
          <motion.div 
            key="loader"
            exit={{ opacity: 0 }}
            className="h-[300px] md:h-[400px] flex items-center justify-center bg-white"
          >
          ...
          >
            {/* Header section - Improved for Mobile Stack */}
            <div className="flex flex-col md:flex-row justify-between items-center md:items-end mb-12 gap-6 border-b border-gray-100 pb-6 text-center md:text-left">
              <div>
                <p className="text-[9px] uppercase tracking-[0.3em] font-bold text-purple-400 mb-1">Impact Section Analysis</p>
              ...
              <div className="flex flex-col items-center md:items-end leading-none">
                <span className="text-[10px] font-black text-gray-300 block mb-2 tracking-widest">FILE: 122_REV</span>
                <span className="text-[9px] font-bold py-1 px-3 bg-purple-50 text-purple-600 rounded-full uppercase border border-purple-100">Live Data</span>
              </div>
            </div>
           ...
           <div className="grid grid-cols-2 lg:grid-cols-4 gap-y-10 gap-x-4 sm:gap-x-8">
            ...
            {/* Footer - Hidden on very small screens or tightened */}
            <div className="mt-16 pt-6 border-t border-gray-50 flex flex-col md:flex-row justify-between items-center gap-4 opacity-30">
               <p className="text-[8px] font-bold tracking-[0.3em]">EST. 2026 ANALYTICS</p>
