...
const CAMPAIGNS = [
  {
    path: "aanna-seva", // The folder name
    category: "ANNAPURNA",
    title: "Aanna Seva",
    desc: "Providing daily nutritious meals to the homeless, laborers, and low-income families across Pune to fight hunger.",
    image: "/WhatsApp Image 2026-04-28 at 3.39.06 PM (9).jpeg",
  ...
  },
  {
    ...
    title: "Swasthya Seva",
    desc: "Providing medical aid, healthcare camps, and emergency support to those who cannot afford essential treatments.",
    image: "/WhatsApp Image 2026-04-28 at 3.39.06 PM (6).jpeg",
    raised: "₹0",
    goal: "₹15 Lakhs",
    progress: "0%",
    color: "#7C3AED",
    ...
    <section className="py-20 bg-gray-50 dark:bg-black">
      <div className="max-w-7xl mx-auto px-6">
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-y-8 gap-x-6 justify-items-center">
          {CAMPAIGNS.map((item, index) => (
            <div key={index} className="flex flex-col items-center w-full">
              <h3 className="text-xl font-bold mb-2 uppercase tracking-widest text-purple-600">
                {item.category}: Campaign
              </h3>

              <CardContainer className="inter-var">
                {/* Updated path to /campaign/ (singular) to match your folder */}
                <Link href={`/campaign/${item.path}`} className="block">
                  <CardBody className="bg-white dark:bg-gray-900 relative group/card border-black/[0.1] dark:border-white/[0.2] w-auto sm:w-[30rem] h-auto rounded-2xl p-6 border shadow-sm transition-all cursor-pointer">
                    
                    <CardItem translateZ="50" className="text-2xl font-bold text-[#1a2b4b] dark:text-white">
                      {item.title}
                    </CardItem>

                    <CardItem as="p" translateZ="60" className="text-sm mt-2 font-medium" style={{ color: item.color }}>
                      {item.desc}
                    </CardItem>

                    <CardItem translateZ="100" className="w-full mt-4">
                      <img src={item.image} className="h-60 w-full object-cover rounded-xl group-hover:shadow-xl transition-all" alt={item.title} />
                    </CardItem>

                    <div className="flex justify-between items-center mt-10">
                      <div className="flex flex-col">
                        <span className="text-[10px] font-bold text-gray-400">Raise: {item.raised}</span>
                      </div>
                      <div className="flex flex-col items-end">
                        <span className="text-[10px] font-bold text-gray-400">Goal: {item.goal}</span>
                      </div>
                    </div>
